  ...
  return (
    ...
    >
      <div className="mx-auto grid w-full max-w-[1440px] grid-cols-1 gap-4 lg:grid-cols-2 lg:items-center lg:gap-12 xl:gap-16">
        {/* Mobile: stacked portraits. Desktop: two-up beside copy */}
        <div className="flex flex-col gap-8 md:grid md:grid-cols-2 md:gap-6 lg:gap-8">
          {founders.map((person, index) => (
            <figure key={person.name} className="flex flex-col gap-6 xl:gap-8">
              <div className="relative aspect-[312/353] w-full overflow-hidden rounded-2xl bg-[var(--color-bg-tint)] md:aspect-[3/4] lg:aspect-auto lg:h-[380px] xl:h-[420px] 2xl:h-[471px]">
                <Image
                  src={person.image}
                  alt={person.name}
                  fill
              ...
              <figcaption className="flex flex-col gap-1.5 px-1 md:px-4">
                <p className="font-[family-name:var(--font-fraunces)] text-[28px] font-semibold leading-9 text-[var(--green-900)]">
                  {person.name}
                </p>
                <p className="text-[13px] font-semibold leading-4 tracking-[1.5px] text-[var(--color-text-on-accent)]">
                  {person.role}
                </p>
              </figcaption>
            </figure>
